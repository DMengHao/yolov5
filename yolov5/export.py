...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
        ...
        conf_thres=0.4  # TF.js NMS: confidence threshold
        ):
    t = time.time()
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/dk01.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/temp_crossing01.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--workspace', type=int, default=16, help='TensorRT: workspace size (GB)')
    parser.add_argument('--nms', action='store_true', help='TF: add NMS to model')
    parser.add_argument('--agnostic-nms', action='store_true', help='TF: add agnostic NMS to model')
    parser.add_argument('--topk-per-class', type=int, default=100, help='TF.js NMS: topk per class to keep')
    parser.add_argument('--topk-all', type=int, default=100, help='TF.js NMS: topk for all classes to keep')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='TF.js NMS: IoU threshold')
    parser.add_argument('--conf-thres', type=float, default=0.4, help='TF.js NMS: confidence threshold')
...
def main(opt):
    for opt.weights in (opt.weights if isinstance(opt.weights, list) else [opt.weights]):
        run(**vars(opt)) #**运算符将字典的键值对作为关键字参数传递给函数
